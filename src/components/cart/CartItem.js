...
const CartItem = ({ item, increment, decrement, deleteFromCart }) => {

    if (!item) {
        return (<div></div>);
    }
    return (
        <div className="flex gap-x-4 items-center justify-between *:flex-[0_0_14%] *:text-center">
            ...
            <h4 className="text-lg font-bold">{ item.price ? `$${item.price}` : "" }</h4>
            <div className="flex items-center justify-center">
                <div className="flex items-center justify-center gap-x-4 py-2 px-4 w-min rounded-lg" style={{ color: "#807D7E", background: "#F6F6F6",  }}>
                    <button className="p-2" onClick={ (e) =>  { e.preventDefault(); decrement({...item}) } }>
                    ...
                    <button className="p-2" onClick={ (e) =>  { e.preventDefault(); increment({...item}) } }>
            ...
            <h4 className="text-lg font-bold" style={{ color: "#BEBCBD", }}>{ item.shipping === 0 ? "FREE" : `$${item.shipping}` }</h4>
            <h4 className="text-lg font-bold">{ item.price && item.amount ? `$${item.price * item.amount + item.shipping}` : "" }</h4>
            <div className="flex items-center justify-center">
                <button className="flex justify-center p-2" onClick={ (e) =>  { e.preventDefault(); deleteFromCart({...item}) } }>
