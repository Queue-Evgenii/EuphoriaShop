...
class ProductItem extends Component {
    render() {
        return (
            <a href={ this.props.item.url } className="rounded-lg p-3 transition hover:bg-gray-100">
                <div className="rounded-lg overflow-hidden mb-4 relative">
                    <img src={ this.props.item.src } alt="" className="h-full w-full object-cover object-center" />
                ...
                <div className="flex item-center justify-between gap-x-4">
                    <div style={{ width: "60%", }}>
                        <h5 className="text-lg font-bold truncate">{ this.props.item.title }</h5>
                        <span className="text-sm">{ this.props.item.brand }</span>
                    </div>
                    <div className="text-lg font-bold py-2 px-4 rounded-lg h-min" style={{ background: "#F6F6F6", }}>{ "$" + this.props.item.price }</div>
                </div>
            </a>
        );
    }
